...
namespace Logica
{
    public class ClsLogicaFactura
    {
        ...
        public DataTable listarFacturas(DateTime fecha)
        {
            ClsDatosFactura factura = new ClsDatosFactura();
            String sql = "SELECT factura.id_factura AS Num, factura.total AS TOTAL, efectivo AS EFECTIVO, tarjeta AS TARJETA  FROM factura WHERE factura.fecha = '" + fecha.ToString("yyy-MM-dd") + "'";
            return factura.consulta(sql);

        }

        public Boolean validarFacturasCanceladas(DateTime fecha)
        {
            ClsDatosFactura factura = new ClsDatosFactura();
            String sql = "SELECT * FROM factura WHERE factura.fecha = '"+fecha.ToString("yyy-MM-dd")+"' AND factura.id_estado_factura < 3";
        ...
        public String agregarPagoFactura(int idFactura, double Pefectivo, double Ptarjeta)
        {
            ...
            public DataTable mostrarFacturasSinCancelarparaPanel(DateTime fecha)
        {
            ClsDatosFactura factura = new ClsDatosFactura();
            String sql = "SELECT factura.id_factura AS ID, factura.id_estado_factura AS ESTADO FROM factura WHERE factura.fecha = '" + fecha.ToString("yyy-MM-dd") + "' AND (id_estado_factura = 1 OR id_estado_factura = 2)";
        ...
        public DataTable mostrarFacturasparaPanel(DateTime fecha)
        {
            ClsDatosFactura factura = new ClsDatosFactura();
            String sql = "SELECT factura.id_factura AS ID, factura.id_estado_factura AS ESTADO FROM factura WHERE factura.fecha = '"+fecha.ToString("yyy-MM-dd") + "'";
            Console.WriteLine(sql);
            return factura.consulta(sql);
        }
