...
{
    public partial class CduPestanaKardex : UserControl
    {
        int tipo_usuario;
        VillaSofia vs;
        ...
        public CduPestanaKardex(VillaSofia vs, int tipo_usuario)
        {
        ...
        private void btnGenerar_Click(object sender, EventArgs e)
        {
            ...
            if (x == -1)
            {
                MessageBox.Show("El insumo insertado no existe");
                dgvInsumo.Rows.Clear();
            }
            else
            {
                ClsLogicaInventario inventario = new ClsLogicaInventario();
                dgvInsumo.DataSource = inventario.generarKardex((int)insumos.Rows[x][4], dtpInicio.Value, dtpFin.Value);
            }
